...
import { updateDisplayIds } from './usersSlice';

const CoasterBtns = ({allUsersIds}) => {
    // ***required states***
    const dispatch = useDispatch()
    

  const usersPerPage = 10;
  //divide allUsers length by amount of users to be displayed per page
  const division = (allUsersIds.length/usersPerPage);
  let pageCount = Math.floor(division);
  const pageBtnWidth = 30;
  //if division not integer increment page by (1) 
  Number.isInteger(division)? pageCount = pageCount += 0 : pageCount +=1; 
  ...
  const handlePageListUpdate = (e,pageNum)=>{
    ...
    const DisplayIds = allUsersIds.slice(firstUserIndex,lastUserIndex);
    dispatch(updateDisplayIds(DisplayIds))
